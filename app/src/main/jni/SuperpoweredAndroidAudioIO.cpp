...
static short int *normalProcessing(SuperpoweredAndroidAudioIOInternals *internals)
{
	...
	{ // If audio input is enabled.
		...
		{ // if we have enough audio input available
			...
			audioGoesToOutput = NULL; // dropout, not enough audio input
	} else
	{ // If audio input is not enabled.
		...
		if ((buffersAvailable + 1) * internals->buffersize < internals->latencySamples)
			audioGoesToOutput = NULL; // dropout, not enough audio generated
	};
...
static void *audioProcessingThread(void *param)
{
	...
	while (!internals->stopAudioProcessingThread)
	{
		...
		if (internals->stopAudioProcessingThread)
			break;
		if (!internals->hasOutput)
			inputOnlyProcessing(internals);
		else
			while (normalProcessing(internals) == NULL)
			{ ;
			};
	};
...
SuperpoweredAndroidAudioIO::SuperpoweredAndroidAudioIO(int samplerate, int buffersize, bool enableInput, bool enableOutput, audioProcessingCallback callback, void *clientdata, int streamType, int latencySamples)
{
	...
	if (latencySamples < 0)
	{ // If latencySamples is negative, a separate processing thread is created for devices with badly configured/poorly written Android Audio HAL. It may help.
	...
	{ // Create the audio input buffer queue.
		...
		if (streamType != SL_ANDROID_STREAM_VOICE)
		{
			...
			{
				SLuint32 presetValue = SL_ANDROID_RECORDING_PRESET_VOICE_RECOGNITION;// SL_ANDROID_RECORDING_PRESET_CAMCORDER; //SL_ANDROID_RECORDING_PRESET_VOICE_RECOGNITION;
